...
用三種完全不同的方法解同一個小型組合最佳化問題：**傳統搜尋**、**微調過的視覺語言模型**（直接讀截圖）、
以及**用模仿學習訓練出來的神經策略**（一步一步把路走出來）。三者由同一個 FastAPI 服務提供，
外加 Gradio 主控台與 Svelte 畫布編輯器，**整套服務一行指令就起得來**。

**重點是比較，不是排行榜**：精確解法在速度與正確性上早就贏了，所以學習方法的價值在於回答
...
| **一個模型三種盤面** | 單一策略同時服務 4×4／5×5／6×6。單次 deterministic 在**三個盤面都贏過**用同資料訓練的單尺寸專用模型（+0.032 ~ +0.037）；給到 best-of-32 的預算時兩者收斂，但單一模型在**每個盤面都用更少的嘗試次數**，訓練成本也比三個加起來略低 |
