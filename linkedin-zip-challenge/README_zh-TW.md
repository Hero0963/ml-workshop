...
*   **從截圖讀題**: 上傳謎題截圖，由本機的微調視覺語言模型讀出盤面後直接求解（Gradio 的 `Solve from Screenshot` 分頁與 `POST /api/vision/solve`）。操作方式見 [`ai-collab/vlm-operating-guide.md`](ai-collab/vlm-operating-guide.md)。
...
*   **Solve from Screenshot (從截圖解題)**: 上傳一張謎題截圖，視覺語言模型讀出盤面後由求解器解題。回應會附上**警告**與**是否有解**的信心旗標——讀錯是會發生的，這兩項就是用來讓你察覺。
...
*   **更難的視覺評估集**: 目前的合成評估集已經飽和（所有指標皆為 1.000），無法再區分不同做法的優劣。要繼續改進讀圖，必須先讓合成資料變難（視覺雜訊、多種渲染風格、模擬截圖失真、更大盤面）。
