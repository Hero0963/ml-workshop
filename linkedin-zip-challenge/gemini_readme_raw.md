...
Zip 是一款推理遊戲，目標是要畫出一條穿過每個格子的路徑，並由小到大依序串連所有數字。

### 規則
*   畫出一條穿過所有可走格子的路徑，**且每個格子只能訪問一次，不能重複**。
*   路徑必須**依序**串起所有標有數字的格子（1 -> 2 -> 3 ...）。
*   如果格子周圍有牆壁（`|` 或 `—`），路徑無法穿越它們。

----

## 目前進度 (Current Progress)

我現在在嘗試用 RL 的方式解 6x6 的地圖問題。

目前的策略是先在**單張地圖**上進行測試並解決問題（過擬合），成功後再挑戰**泛化**到任意 6x6 地圖的最終目標。

開發的起點是從 `src/core/rl/train_single_sb.py` 這個腳本的實作開始。
