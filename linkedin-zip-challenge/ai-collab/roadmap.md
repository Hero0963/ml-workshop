...
3. **RL solver 重啟** ← **★ 進行中：A0／A1／A2 ＋ 續訓都跑完了，卡在「推到全長值不值得」**
   ...
   - **★ 優先序重排（2026-09-05 本人指示：回到原本的 goal）**，完整版在
     ...
     **P0 只做一件——連通性特徵**（未訪區域的連通分量數當純量特徵）。
     機制：held-out 失敗 **59.1% 是死路**，而 env 只在四方向全擋時才終止（局部死路，太晚）；
     未訪區域一分裂就已無解，但策略不知道。成本已量 **+4.6%**。
     Done：3 seed × 2 臂，效果**大於 ±0.04** 才算有效。
