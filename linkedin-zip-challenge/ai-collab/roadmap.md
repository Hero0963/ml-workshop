...
| RL solver（`src/core/rl/`） | 🚧 **重啟中**：A0／A1（2026-08-15）＋ **A2 兩輪訓練完成（2026-08-29）**。資料集加大 11 倍後 held-out **4×4 0.788→0.877、6×6 0.253→0.344**，**訓練／held-out 落差從 +0.162／+0.250 收斂到 +0.050／+0.009** ⇒「在背答案」已解掉。**兩個 goal 仍未達門檻**；下一步是 **6×6 加訓練預算**（curriculum 只到 k=27/36 且還在爬）＋ **shaping 對照** |
...
3. **RL 重啟前置研究** ← **★ VLM 之後接這個（可與 VLM 交錯進行：VLM 等 Colab 時本機推 RL）**
   ...
   - **⚠ 發現一個沒人記錄的設計偏離**：restart 報告的階段表指定 shaping λ 在**一筆畫階段是 0**（「只剩 +1 與 γ」），
     ...
     實算佔一局總分 **14%**，從未關掉跑過。
   - **下一步（依序）**：① **6×6 用 `--resume` 加預算**（`--timesteps 3000000` 從 5M 續到 8M，約 12 分鐘）；
     ② **`shaping_lambda=0` 的對照**（回應上面那個規格偏離，一次跑就有答案）；
     ③ 之後才談調超參／改網路容量。**資料量這條線已經走完，不要再加資料。**
