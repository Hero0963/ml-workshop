...
3. **RL solver 重啟** ← **★ 2026-09-12：兩個 goal 都有達標路徑，solver 已掛上 API，服務可用 Docker 一鍵起**
   ...
   - **🎯 現況一句話**：**兩個 goal 都達標且有餘裕**——目前最好的模型 `bc_multi_456_e6`（BC 只訓練 6 epochs）
     ...
     ⚠ **服務中的仍是 `bc_multi_456`（10 epochs）**，它的 6×6 是 0.8535「剛好踩線」——**要不要換由本人決定**。
   ...
   - **★★ 順著上一條做的追加實驗，是這輪最大的一筆**：BC 只訓練 **6 epochs**（`bc_multi_456_e6`，同 seed 配對比較）
     ...
     **判定基準是 best-of-32 ⇒ 正確方向是「少訓練」。** ⚠ 單一 seed、**沒有掃 epoch**（最佳點可能更早，每點約 17 分鐘）。
     **服務中的仍是 10 epochs 版，要不要換由本人決定。**
