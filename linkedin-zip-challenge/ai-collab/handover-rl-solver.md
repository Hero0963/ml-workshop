...
### ✅ 資料集已加大（2026-08-29），下一步是拿它重訓

**根因是泛化不是訓練量**（§0 ②），所以在資料變多之前，加步數、調超參、改網路都只會讓模型把
同一批 1,360 題背得更熟。**新資料集已經生好並驗過**：

| `seed20300000_n20000_4-6` | train | val | test | 去重丟掉 |
|---|---|---|---|---|
| 4×4 | 15,419 | 1,927 | 1,928 | 726 |
| 6×6 | 16,000 | 2,000 | 2,000 | 0 |

約為 A2 訓練量的 **11 倍**；三個 split 的兩兩交集皆為 **0**；`--verify` 通過。
`DEFAULT_DATASET` 已指向它，**要重現 A2 的數字就加 `--dataset main_n1700_456`**。

**接著要做的**：用它重跑 goal1 與 goal2（各約 4 分鐘／20 分鐘），然後**用同一支診斷比較落差**
（deterministic 同時打訓練集與 held-out，做法見 §0 ②）。

- **Done 條件**：deterministic 的「訓練集 vs held-out」落差明顯縮小（A2 是 4×4 0.947→0.788、
  6×6 0.553→0.253）。**落差沒縮小就不要往下走**——那代表瓶頸不在資料量，
  要改查網路容量或正則化（§0 ② 標明那兩項還沒排除）。
- ⚠ 6×6 的 A2 只推到 k=33/36，所以這次要同時看**兩件事**：落差有沒有縮小、curriculum 有沒有推到全長。
  兩者混在一起會看不出誰造成什麼，必要時先用 `--dataset main_n1700_456` 加大步數當對照。
