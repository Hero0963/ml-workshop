...
### ✅ 上面這一項當天就做完了（2026-09-05），結論見 §0 ⑬ 與報告 §10

**做對的**：軸換成節點預算、放了 `dfs_random` 當先驗的控制組、放了 `dfs_policy_sampled` 當確定性的控制組。
**量到的**：先驗價值很大（6×6 不准回溯時 0.4205 vs 0.0005），但 **6×6 在 100 節點以上輸給 best-of-N**。

⚠ **這一節原本的推薦被自己的數據否掉一半**，教訓寫在報告 §10.6：
我從「獨立重抽很笨」推出「有結構的會更好」，**卻沒先問這個方法假設的失敗模式是什麼**；
而能推翻它的證據（§0 ⑩：59% 的窄化決策已經四路皆敗 ＝ **錯誤在很早**）**當天稍早就量到了、沒有拿來用**。
⇒ **提新方法之前，先把它假設的失敗模式，對照已經量到的失敗模式。**

### ★ 再下一個候選（**未授權**，約 10 分鐘）：帶重啟的 policy-DFS

保留策略先驗排序，但**每 K 個節點從頭重啟一次**（順序改成抽樣，所以每次走的路不同）。

- **低預算時它就是 policy-DFS**（4×4 贏的那段）、**預算變大時重啟會重擲早期決策**（6×6 best-of-N 贏的原因）。
- **兩個要素都已經量過**：抽樣排序只差 −0.03（§3.26）、重擲早期決策在 6×6 有效（§3.25）
  ⇒ **這不是新賭注，是把兩個已知有效的東西接起來。**
- **Done 條件**：6×6 要在 175 與 500 節點**同時**勝過 `best_of_n`，且 4×4 不得比 `dfs_policy` 差。
- **腳本已在**：`../hi-collab/scratch/probe_policy_guided_dfs.py`，加一個臂即可（`--arms` 可只跑新臂）。
