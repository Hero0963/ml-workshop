...
> 其餘去哪查看 **§6 總目錄**。
> 最後更新：2026-09-12（Asia/Taipei）｜分支 `feat/rl-ppo-finetune`（Track A）｜worktree `zip-rl`｜roadmap 第 3 項
>
...
**現在開著的是三件事**：① 要不要把服務模型換成早停版（本人決定）
② **掃 BC 的 epoch 找最佳點**（每點約 17 分鐘，目前最划算）③ **ExIt／拒絕抽樣微調**（保住多樣性的方法）。
**不是在做這三件的工作，開跑前先說明為什麼要做。**
...
| ~~2~~ | ~~**BC 早停**~~ | **✅ 2026-09-12 當天就驗了，而且是目前最大的一筆**：`--epochs 6` 讓 6×6 **best-of-32 從 0.8535 → 0.9465**、每題嘗試 7.76 → 5.24（同 seed 配對比較）。**剩下的是掃 epoch 找最佳點**（每點約 17 分鐘）＋加資料 | 已花 8 分鐘 | ❌ 監督式 |
