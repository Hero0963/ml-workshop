...
雲端 4 核心 CPU 的實測執行時間（2026-09-25，整本 notebook 從頭執行到存檔）：

| notebook | 時間 | 備註 |
|---|---|---|
| 02、03 | 20 秒以內 | 不訓練（03 第一次會下載 MNIST） |
| 04、05 | 約 2 分鐘 | 2D MLP，5000 步 |
| 06 | 約 30 分鐘 | U-Net 16 通道訓練 2000 步約 16 分鐘，其餘是 1000 步取樣（256 張 × 2）。存下的輸出是與其他工作共用 CPU 時跑的，紀錄為 56 分鐘 |
| 07 | 約 7 分鐘 | 載入 06 的模型；1000 步 DDPM 佔 5 分鐘 |
| 08 | 約 23 分鐘 | 條件 U-Net 訓練 2000 步約 14 分鐘 |
| 09 | 約 3.5 分鐘 | 2D，含 reflow |
| 10 | 約 18 分鐘 | TinyDiT 訓練 3000 步約 16 分鐘 |

**不要同時跑兩本訓練型 notebook**：兩個 PyTorch 行程各開 4 個執行緒搶 4 顆核心，會互相拖慢好幾倍（2026-09-25 實際踩到：訓練一步從 0.5 秒變成 8 秒）。
