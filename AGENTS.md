...
| `language-models-course/` | 語言模型自學課程（2026-09-25 建立）：整合 word2vec、GPT-2、Stanford CS336、nanochat、embedding 模型；16 課繁中講義 ＋ 13 本已執行的實驗 notebook（n-gram → BPE → GPT-2 載入真權重 → 預訓練 → scaling／系統／推論／資料 → SFT＋工具 → GRPO／DPO → embedding）＋ 有測試的參考實作，CPU 可跑 | `ai-collab/` |
...
| `language-models-course/.venv` | 3.12 | 該子專案（torch 走 cu126 explicit index） |
...
**環境確定性**：子專案應有 `.python-version` 讓 `uv sync` 決定性挑版本。目前 `thread-the-grid`（3.11）、`board-game-rl`（3.13）、`diffusion-models-course` 與 `language-models-course`（3.12）有；其餘待補。
...
| 語言模型課程（講義／實驗／參考實作） | `language-models-course/ai-collab/roadmap.md` → `project_guide.md` |
