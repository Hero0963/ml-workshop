...
from lm_course.artifacts import BASE_MODEL_PATH, base_model_config, course_tokenizer, course_tokens
...
def main() -> None:
    ...
    parser.add_argument("--lr", type=float, default=3e-3, help="AdamW lr (embeddings, head)")
    ...
    logger.info(f"{count_parameters(model):,} parameters, {len(train_tokens):,} training tokens")
    ...
    final = evaluate(model, val_tokens, model.config.context_length, 32, 40, token_bytes)
    logger.info(
