...
- **程式**（HumanEval）：執行單元測試，報告 pass@k（第 12 課式 12.10）。
...
- **本課程的例子**：Lab 06 §5 的 250 步比較只有一個種子；Lab 12 的加法正確率只在一種題型上量。報告數字時要附上「在什麼條件下」。
...
3. 設計一個實驗，檢查 Lab 12 的模型在「加法」上的進步是否只是學會了特定的提問句型（提示：換一個訓練時沒出現過的問法）。
