...
- **「詞彙越大越好」**：壓縮率變好，但 embedding 參數變多、稀有 token 學不好；小模型尤其吃虧。而且進步會飽和：Lab 03 在 TinyStories 上從 4,096 加到 16,384，每個 token 只多裝了約 0.15 個位元組。
- **「tokenizer 和模型無關，可以隨時換」**：模型的 embedding 表是按 token 編號學的，換 tokenizer 等於換一個模型。
- **「BPE 學出來的都是有意義的詞根」**：它只看頻率，會學出「 blin」「ummer」「 stom」這種片段（Lab 03 看得到）。
