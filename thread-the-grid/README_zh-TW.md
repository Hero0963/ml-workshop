...
一筆畫路徑謎題——一條線走過每一格、依序經過編號格——從頭做到尾：**出題**、從截圖**讀題**、以及**解題**。
三件事由同一個 FastAPI 服務提供，外加 Gradio 主控台與 Svelte 畫布編輯器，
**整套服務一行指令就起得來**。靈感來自網路上的 Zip 遊戲。
...
| | |
|---|---|
| **出題**：4×4、5×5 或 6×6，出完可以直接帶進編輯器 | ![Generate Puzzle 分頁](./illustrations/ui/gradio-generate-puzzle.png) |
| **讀截圖**：微調過的視覺模型讀出盤面（左），solver 畫出路線（右）| ![Solve 分頁，上傳模式](./illustrations/ui/gradio-solve-upload.png) |
| **編輯與解題**：表格填號碼，牆是可以按 × 刪掉的標籤，這張由學出來的策略解開 | ![Solve 分頁，編輯器模式](./illustrations/ui/gradio-solve-editor.png) |
| **Svelte 畫布編輯器**：點格子中間填號碼，點格線切換牆 | ![Svelte 編輯器](./illustrations/ui/svelte-editor-rl-solved.png) |
| **API**（`/docs`）| ![API 文件](./illustrations/ui/api-docs.png) |
...
`POST /api/puzzle/generate` 產生 4×4、5×5 或 6×6 的帶牆盤面，限時搜尋放棄時會自動重試；
Gradio 主控台與 Svelte 編輯器都走這支 API。另有決定性的資料集產生器（`src/core/rl/generate_dataset_v2.py`），
它**把解答一起存下來**，這正是後來模仿學習得以成立的原因。
...
| | |
|---|---|
| **選了哪個模型** | **Qwen3.5-4B**（Apache-2.0），LoRA 微調後由 Ollama 以 `threadgrid-qwen35-4b-p4c:f16` 服務 |
...
⚠ 目標是**讀懂這個專案自己畫的盤面**，數字證明的也是這件事；**不證明**看得懂其他版本遊戲的截圖
...
取第一個通過驗證的——**這在這裡是合法手段**，因為這種題目的解可以自我驗證。第一次成功就停，所以平均成本遠低於 32。
...
| Windows：容器都 healthy，但 `http://127.0.0.1:7440` 沒回應 | `%USERPROFILE%\.wslconfig` 裡的 `networkingMode=mirrored` 搭配較舊的 Docker Desktop（實測 4.32）時，主機上根本沒有程序在聽對外的埠。註解掉那行，再 `wsl --shutdown`——2026-09-19 實測有效。據回報新版 Docker Desktop 在 mirrored 模式下 `localhost` 是通的（[microsoft/WSL#41284](https://github.com/microsoft/WSL/issues/41284)），本專案沒測過 |
...
│   │   ├── routers/          # echo／puzzle／solver／vision 端點
│   │   └── schemas/          # 請求與回應模型
│   ├── core/
...
*   **Generate Puzzle**——隨機出 4×4、5×5 或 6×6 的帶牆盤面。按 **Open in editor** 帶到 Solve 分頁。
*   **Solve**，兩種模式：
    *   **Upload screenshot**——視覺模型讀盤面、solver 解題；答案附 `warnings` 與 `solvable` 旗標。
        按 **Edit in editor** 把讀到的盤面帶進編輯器，讀錯的牆修好再解一次。
    *   **Editor**——在表格裡填號碼（`x` 是障礙格）、用兩個格子座標加牆、按標籤上的 × 刪牆，選 9 種 solver 之一解題。

Svelte 編輯器（`/svelte-ui/`）在畫布上做同樣的事：選好大小後按 **Generate** 出題、按 **Blank grid** 從空白盤開始，或讀截圖；再點格子與格線修改，然後解題。
`GET /api/echo/health` 仍保留給健康檢查用。介面截圖在 [`illustrations/ui/`](./illustrations/ui/)。
...
cd thread-the-grid
uv sync
uv run pytest            # 2026-09-19 實測 344 passed, 8 xfailed（models/ 裡有 RL checkpoint）
uv run ruff check .
```

那 8 個 `xfail` 是**刻意的**：它們釘住已退役的 v1 環境的缺陷，所以「意外通過」代表有人動了它。
`models/` 底下沒有 RL checkpoint 時，需要它的那 1 個測試會被 skip。
...
Copyright 2025–2026 Hero0963。本目錄的程式碼與 RL 策略權重以 [Apache License 2.0](./LICENSE) 授權。
視覺模型是 Qwen3.5-4B 的微調版，Qwen3.5-4B 本身是 Apache-2.0，微調版以同樣的授權發佈。

不在授權範圍內：測試用的真實遊戲截圖（`illustrations/puzzle_01.png` 到 `puzzle_06.png`）屬於各自的權利人；
兩張用來做模型健全性檢查的照片（`illustrations/cat.png`、`illustrations/bird.png`）來源沒有記錄。
