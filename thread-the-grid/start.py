...
* The app serves the code of the checkout it was built from, so there is one per
  ...
  the default -- the name of this directory, which is the same in every worktree -- so
  `up` in one worktree silently replaced the stack another was running.
...
OLLAMA_CONTAINER = "threadgrid_ollama_server"
APP_SERVICE = "threadgrid-app"
...
def stack_name(dev: bool) -> str:
    """This checkout's compose project: `threadgrid-app-<checkout>` or `threadgrid-dev-<checkout>`.
    ...
    return f"threadgrid-{'dev' if dev else 'app'}-{checkout or 'default'}"
