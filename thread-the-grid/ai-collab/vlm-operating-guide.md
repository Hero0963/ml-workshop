...
> ⚠ **Docker Desktop 有開 ≠ 容器有跑。** `threadgrid_ollama_server` 會停在 `Exited`，
> 第二行就是把它叫起來。確認用 `docker ps`，要看到 `threadgrid_ollama_server` 是 `Up`。
...
2. 切到 **`Solve`** 分頁，Mode 選 **`Upload screenshot`**（預設就是它）
3. 拖一張謎題截圖進去（或按貼上）
...
| 解答圖 | 走法（勾了動畫才有 GIF） |
| `Layout read` | 模型讀到的盤面，**Python literal** |
| `Walls read` | 模型讀到的牆，同樣是 Python literal |

**後兩塊是為了「模型讀錯時你能自己修」而存在的**：按下方的 **`Edit in editor`**，讀到的盤面會帶進 Editor 模式，
改掉錯的那一格、或按牆標籤上的 × 拿掉多讀的牆，再解一次（2026-09-19 起；以前要手動貼到 Naive／Interactive 分頁，那兩個分頁已合併掉）。
Svelte 編輯器（`/svelte-ui/`）的 **Read screenshot** 也一樣：讀到的盤面直接放上畫布，點格線就能增刪牆。

**示範**：`illustrations/puzzle_03.png` 會被多讀一道牆 `((1, 2), (2, 2))`（「1」的正下方）而回報無解；
在編輯器拿掉它，CP-SAT 就解得開（2026-09-19 實測）。
...
| **503** | 模型連不上 | 回 §1 把 `threadgrid_ollama_server` 叫起來 |
...
| 回 503 | `threadgrid_ollama_server` 沒跑 |
...
```powershell
docker exec threadgrid_ollama_server sh -c "printf 'FROM /models/gguf/zip-qwen35-4b-p4c-text-f16.gguf\nFROM /models/gguf/zip-qwen35-4b-p4c-mmproj-f16.gguf\n' > /tmp/Modelfile && ollama create threadgrid-qwen35-4b-p4c:f16 -f /tmp/Modelfile"
docker exec threadgrid_ollama_server ollama show threadgrid-qwen35-4b-p4c:f16
```
...
{"file_name": "images/20260829-044717_949e2bfc.png",
 ...
 "model_name": "threadgrid-qwen35-4b-p4c:f16", "prompt_variant": "finetune",
...
| 症狀 | 多半是什麼 | 怎麼修 |
|---|---|---|
| API 回 **503** | `threadgrid_ollama_server` 沒跑 | `docker compose -f docker-compose.ollama.yml up -d` |
...
| 每次都很慢、答案還很爛 | 模型 tag 是未微調的那個 | `docker exec threadgrid_ollama_server ollama list` 對一下，再看 §5 |
