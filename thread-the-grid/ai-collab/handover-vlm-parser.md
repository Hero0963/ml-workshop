...
> 🛑 **2026-09-19 專案收尾，這條 track 已停止。** 目標「解我們自己做的圖」達成：收尾當天用全新生成的 6 張＋held-out 4 張再驗一次，**10/10**（[收尾報告](reports/2026-09-19_project-wrap-up.md) §2、§7）。模型權重（9.1 GB）**不在版控**，2026-09-19 公開在 [Hugging Face](https://huggingface.co/Hero0963/threadgrid-qwen35-4b-p4c-gguf)，安裝步驟見 [`model-weights.md`](model-weights.md)——含一個坑：匯出的文字塔 GGUF 與 Ollama blob 雜湊不同，但 426 個張量逐位元組相同（Ollama 匯入時重排順序）。Ollama 已釘在驗證過的 **0.32.13**。
...
> merge 進 base、轉成 GGUF、由 Ollama 以 `threadgrid-qwen35-4b-p4c:f16` 服務，
> 並由 `POST /api/vision/solve`、Gradio 的 `Solve` 分頁（Upload screenshot 模式）與 Svelte 的 Read screenshot 使用。
...
Copy-Item .\thread-the-grid\.env ..\zip-vlm\thread-the-grid\.env   # .env 不進版控，必須複製

# 每次開工
cd D:\it_project\github_sync\zip-vlm\thread-the-grid   # ★ 一定要進子專案再跑 uv
uv sync
...
> 一律 `cd thread-the-grid` 之後才 `uv run`。
...
docker exec threadgrid_ollama_server ollama --version     # 應為 0.32.13 或更新
docker exec threadgrid_ollama_server ollama list          # 應看到 threadgrid-qwen35-4b-p4c:f16（微調版）與 qwen3.5:4b-q8_0 等
```

- 容器名 **`threadgrid_ollama_server`**、host 埠 **11435**（不是 11434，11434 被本機另一專案佔用，**不要改回去**）。
- 模型放在具名 volume `linkedin-zip-challenge_ollama_data`，**所有 worktree 共用同一份**，不必重拉。
- ⚠ 2026-09-12 起 **ollama 是整台機器一個**（compose 專案 `threadgrid-ollama`），app 則是**每個 checkout 一組**
  （`threadgrid-app-<checkout>`），兩個 checkout 可以同時起——各自在 `.env` 用不同的 `APP_PORT` 即可。
...
- adapter merge 進 base 後轉 GGUF，由 Ollama 以 **`threadgrid-qwen35-4b-p4c:f16`** 服務（文字塔 8.42 GB ＋ mmproj 672 MB）。
...
| 陷阱 | 說明 |
|---|---|
| **`uv` 路徑** | repo 根 `.venv` 是 py3.9 devtools。一律 `cd thread-the-grid` 再 `uv run` |
