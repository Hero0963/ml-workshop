...
> Chronological development history of `thread-the-grid` (named `linkedin-zip-challenge` until 2026-09-19), **newest first**.
...
- **只留 main**（2026-09-20）：
  ...
  3. 刪除的分支（全部是 `main` 的祖先，SHA 留在這裡，要找回用 `git branch <名稱> <SHA>`）：
     ...
     | `feat/thread-the-grid-round-2` | `3b25e79` | 遠端（合併進 main 後）＋本機（`zip-vlm` 移除後）|
...
**本人當次指示**：修好 Windows 連不進 Docker、實際操作截圖、處理權重上傳，最後更新文件；途中陸續定案 UI 改版、授權、改名。
總帳在 [`reports/2026-09-19_wrap-up-round-2.md`](reports/2026-09-19_wrap-up-round-2.md)，決策與步驟在 [`plans/2026-09-19_wrap-up-round-2.md`](plans/2026-09-19_wrap-up-round-2.md)。

- **埠轉發**：根因是 2026-09-18 另一個 agent 加的 WSL `networkingMode=mirrored`（當成可選加分項）配 Docker Desktop 4.32；改回 NAT 後對照組 200。
  第一輪報告把 microsoft/WSL#41284 當成衝突證據是誤讀——那個 issue 裡新版 Docker Desktop 的 localhost 是通的。
- **Gradio**：兩個分頁、出題走新 API `/api/puzzle/generate`、讀圖可帶進編輯器、牆改多選標籤。途中二分出兩個 Gradio 前端 bug，
  升 6.17.3（本人 `uv lock`；6.18+ 需 `huggingface-hub>=1.x` 與 `transformers<5` 衝突）。
- **Svelte**：出題、空白盤、讀截圖；本人驗收抓到「Generate 後畫布空白」與「兩處都要設大小」——前者修（DOM 更新後再畫，且不能對元件 `ctx` 做成員賦值，否則無限重畫），後者照本人選擇共用一組大小。
- **授權**：子專案 Apache-2.0；README 排除 LinkedIn 截圖與來源不明的 cat／bird 測試圖。
- **改名**：查到 LinkedIn 已申請 ZIP 商標（USPTO 99060408，2026-08-11 公告）⇒ 名稱連 zip 都拿掉，`thread-the-grid`／`threadgrid`；README 只寫「Inspired by the Zip puzzle games found online」。資料夾逐項搬（工作目錄不能改名），舊 `.gitignore` 規則保留。
- **權重**：RL 建成 draft release（SHA-256 核對、匿名 404）；VLM 上傳資料夾與 model card 備妥，等本人登入 HF。
...
**基線**：`uv run pytest` **280 passed, 8 xfailed in 28.59s**、`uv run ruff check .` 全綠（與 2026-09-12 記錄一致）。

**計畫與順序**（每階段做完就回來補數字）：

| 階段 | 做什麼 | done 條件 |
|---|---|---|
| 1 | 掃 epoch（handover §3 的配方）| sweep 的 e6 重評復現 **0.9465**；`bc_progress.jsonl` 前六行與已發表的逐位相同 |
| 2a | ExIt 前置檢查：同一題**真的有別條解**嗎 | 量出「解開的路徑 ≠ 資料集那條」的比例；幾乎沒有就用數據把 ExIt 結案 |
| 2b | ExIt 第一輪 | 與階段 1 **同 seed、同步數、同題目權重**，唯一差別是標籤 |
| 3 | 贏家多跑 2 個 seed | 順便清掉「6×6 seed 雜訊從沒量過」 |

**階段 1 進度**：
- 程式：`train_behaviour_cloning.py` 加 `--checkpoint-every-epoch`（存 `model_epoch_<N>.zip`），
  補測試 `test_saving_every_epoch_leaves_the_training_trajectory_alone`（同 seed 兩次訓練、一次中途存檔，loss 與權重逐位相同）；
  `../hi-collab/scratch/probe_cross_size.py` 加 `--checkpoint`／`--label`（**評分迴圈沒動**；有 `--checkpoint` 就強制要 `--label`，避免覆蓋已發表產物）。
- 2026-09-19 01:29 開跑 `bc_multi_456_sweep`（`goal3_multi`、10 epochs、每 epoch 存檔），號誌 `busy rl`。
  **決定性複驗通過（10/10）**：十個 epoch 的 loss 與 val 準確率和 `bc_multi_456` 已發表的**逐位相同**，
  連結尾的 deterministic 評估（`eval_test.json` 的 `per_size`：4×4 0.94045／5×5 0.74963／6×6 0.5205）也一模一樣
  ⇒ **sweep 的 e10 就是已發表的 `bc_multi_456`**，它的 6×6 best-of-32 **0.8535** 可以直接當 e10 這一點，不必重跑。
- ⚠ **GPU 預算踩線兩次、都先停再修**：
  ① 同時開 5 個 6×6 probe（e1–e5）＋訓練 ⇒ GPU 使用率 **98%**（CPU 只有 45%）。停掉 e1／e2／e3／e5（各跑了 1–3 分鐘，**結果沒用**），
  剩訓練＋1 個 probe 時是 33–49%。② 以為「2 個 probe」可以 ⇒ 實測 **84–88%**（`nvidia-smi pmon`：兩個 probe 各約 40%），
  又停掉 e2。**原因**：probe 一次只餵一個觀測，GPU 被極小的 kernel 塞滿；**有空核心時一個 probe 約佔 40% GPU 時間**
  ⇒ **同時只能 1 個 probe**（實測 1 個 probe ＋ CPU 收集時 GPU 20–33%、CPU 52–66%）。
  ⚠ 教訓：**同一件事的 GPU 佔用會隨 CPU 爭用變動**（CPU 被搶時 probe 變慢、GPU 反而閒），所以預算要在「實際要跑的組合」下量，不能從別的組合推。
...
**階段 2a：ExIt 前置檢查——通過，而且很明顯**（`logs/rl_exit/gate_6x6_train500_k32/summary.json`，收集者 = 服務中的 `bc_multi_456_e6`）：
6×6 **訓練集**前 500 題、每題抽 32 次：
...
| 解開的題平均相異解數 | **5.04** |
| 資料集那條解有出現在抽樣中 | 0.909（解開的題）|
| 別條解總數 | 2,047（平均每題 4.1 條）|
| **被獨立檢查器（`calculate_fitness_score`）退件** | **0** |

⇒ **「這些題其實多解」從推論（88.11% vs 89.47% 的落差）變成實測**：6×6 訓練題 87% 有別條解，而 BC 每題只看過一條。
ExIt 有東西可加。25.7 秒（CPU、1,024 局並行）。
- 2026-09-19 01:46 開始全量收集 `exit_r1_e6_k32`：訓練集 47,435 題 × 32 次（1,517,920 局，CPU，**1,927 秒**）。
  **三個尺寸**（`logs/rl_exit/exit_r1_e6_k32/summary.json`；「有別條解」是**下界**——只數得到模型自己抽得到的）：

  | 盤面 | 題數 | 有別條解 | 解開的題平均相異解數 | 資料集那條有被抽到 | 獨立檢查器退件 |
  |---|---|---|---|---|---|
  | 4×4 | 15,439 | 55.8% | 2.37 | 99.5% | 0 |
  | 5×5 | 15,996 | 76.4% | 4.06 | 96.2% | 0 |
  | 6×6 | 16,000 | 88.2% | 5.34 | 89.6% | 0 |

  ⚠ **多解不是「題目出得好」**：出題器是「先隨機畫一條哈密頓路、再挑 1/4–1/3 的格子標數字、隨機加牆」，
  **完全不檢查唯一解**（`puzzle_generator.py:74-125`）。真的 LinkedIn 題是否唯一解**沒查證**——若是，ExIt 的好處未必轉移得到真題。

**★ 本人追問「判定器有問題嗎」⇒ 查出一個全專案的寬鬆點（影響小，已在 ExIt 端處理）**：
env 的 `_is_solved`、`dfs.py`、`calculate_fitness_score` **三個判定器都不檢查終點**——
「走過最大數字後繼續把剩下格子走完」也算解開。規則本身是模糊的：LinkedIn 官方說明只寫「填滿所有格子、依序經過數字、不穿牆」，
**沒說**終點必須是最大數字；第三方教學站則寫「停在最大數字」；本專案出題器**永遠**把最大數字放在路的最後一格（資料集 0 例外）。
實測影響（`logs/rl_exit/endpoints_exit_r1_e6_k32.json`，`../hi-collab/scratch/analyse_endpoints.py`）：
收集到的別條解有 **4×4 5.5%／5×5 4.0%／6×6 3.6%** 沒停在最大數字；6×6「有別條解」88.2% → **87.5%**；
服務中的 `bc_multi_456_e6` 在 6×6 測試集 best-of-32 **寬鬆 0.948 → 嚴格 0.9435（−0.0045，在 ±0.01 評估雜訊內）**。
**處理**：`collect_solutions.py` 只收「停在最大數字」的解（新增 `ends_on_last_number`，另計 `off_last_number`）；
既有收集用 `../hi-collab/scratch/filter_strict_solutions.py` 過濾成 `solutions_strict.json`
（過濾不影響抽樣的亂數流，所以等同重跑）：**141,505 → 135,818 條、34,963 → 34,364 題**。
**沒改全專案判定器**（`dfs.py` 屬 Track C、`utils.py` 只讀）——要不要全面改成嚴格版由本人決定；
⚠ 服務中的 RL solver 約 4% 相異解停在別處，嚴格規則下 API 偶爾會回「我們判對、嚴格版不算」的路。
...
|---|---|---|---|
| 4×4 | **0.590**（1,139／1,931） | **1.000**（200／200，95% CI 0.981–1.000） | 0.9953 |
| 6×6 | **0.000**（0／2,000） | **0.030**（6／200，95% CI 0.014–0.064） | 0.9465 |
...
**4×4 包了預算其實不差**（中位數第 1 次就解出，和 RL best-of-32 同水準）；**不開放的理由全在 6×6**：
5 秒能試約 107 次，200 題只解出 6 題。兩欄彼此一致：6×6 總共 21,407 次解出 6 題，單次約 0.00028，
落在單次呼叫 0／2,000 的 95% 上界 0.0015 之內。

**兩欄為什麼分開量**：「包 5 秒預算」是牆鐘預算，第一次要量時 `.agent-heavy-job` 是 `busy rl 2026-09-19 01:29`（CPU 約 50%），
依 AGENTS §10.3 不搶資源，而且別人佔著 CPU 時分數會被壓低、不能用。單次呼叫那欄每題固定 seed、與機器忙不忙無關，
所以先量（單一 process，137.8 秒）。號誌在 02:53 空出來後改成 `busy solvers`，8 個 worker 跑 127.0 秒，
執行中取樣 CPU 36–44%，跑完改回 `free`。
