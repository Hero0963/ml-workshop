...
|---|---|---|---|---|
| `threadgrid-app-<checkout>-threadgrid-app-1` | `threadgrid-app-<checkout>`（**每個 checkout 一個**）| FastAPI ＋ Gradio ＋ 已建好的 Svelte 編輯器 ＋ **9 種 solver（含 RL）**（2026-09-12 時是 4 種，Track C 之後全部上線、PSO 下架） | `APP_PORT`（預設 `7440`）| ❌ 不用 |
| `threadgrid_ollama_server` | `threadgrid-ollama`（**整台機器一個**）| 服務微調過的視覺語言模型，`/api/vision/solve` 靠它讀截圖 | `11435`（對外）→ `11434`（容器內）| ✅ 要 |

`<checkout>` 是 `thread-the-grid` 的**上一層目錄名**：主 checkout 是 `ml-workshop`，worktree 是 `zip-infra`、`zip-rl`……
所以在 `zip-infra` 裡，app 容器叫 `threadgrid-app-zip-infra-threadgrid-app-1`。
...
|---|---|---|
| compose 專案名 | 預設＝compose 檔所在的**目錄名** | 每個 worktree 的子目錄都叫 `thread-the-grid` |
...
| | 專案名 | 誰決定 | 行為 |
|---|---|---|---|
| ollama | `threadgrid-ollama` | 寫死在 `docker-compose.ollama.yml` 的 `name:` | **刻意的單例**。`start.py` 只在它沒跑時才起，**在跑就完全不碰** |
| 正式 app | `threadgrid-app-<checkout>` | `start.py` 的 `stack_name()`，用 `-p` 傳給 compose | 每個 checkout 各自的容器、網路、image |
| 開發 app | `threadgrid-dev-<checkout>` | 同上 | 同一個 checkout 的 prod／dev 用同一個埠，`start.py` 起一個時會先停另一個 |

幾個細節，都是實測出來的：

- **為什麼在跑的 ollama 一定不能碰**：ollama 掛了 `./models`，那是「當初起它的那個 checkout」的路徑。
  從別的 checkout 對它 `up`，compose 會認為設定變了而**重建**它，把別人正在用的模型卸掉。實測：
  `docker compose --dry-run -f docker-compose.ollama.yml up -d` → `Container threadgrid_ollama_server  Recreate`。
...
- **不用 `start.py` 的時候**：專案名退回預設，正式版是目錄名 `thread-the-grid`、開發版是檔案裡寫的
  `threadgrid-dev`（兩者分開，image 才不會互相覆蓋）。單一 checkout 沒問題；**多個 worktree 請用 `start.py`**
...
### 從 `zip-*` 名稱遷移（2026-09-19 專案改名 `thread-the-grid`，每台機器一次）

專案資料夾改名，容器與 compose 專案也跟著改：`zip_ollama_server` → `threadgrid_ollama_server`、`zip-ollama` → `threadgrid-ollama`、
`zip-app-<checkout>` → `threadgrid-app-<checkout>`、模型標籤 `zip-qwen35-4b-p4c:f16` → `threadgrid-qwen35-4b-p4c:f16`。
**volume 名稱不變**（還是 `linkedin-zip-challenge_ollama_data`，Docker 不能改 volume 名，換名就等於模型全部重下）。

```powershell
# 1. 停掉舊名稱的容器（它們佔著 7440 / 11435）
docker stop zip_ollama_server
docker compose -p zip-app-<checkout> down
# 2. 模型複製一份新標籤（同一份資料，不多佔空間；舊標籤留著無妨）
python start.py                       # 會起 threadgrid_ollama_server（同一個 volume）
docker exec threadgrid_ollama_server ollama cp zip-qwen35-4b-p4c:f16 threadgrid-qwen35-4b-p4c:f16
# 3. .env 的 OLLAMA_MODEL_NAME 改成 threadgrid-qwen35-4b-p4c:f16（.env 不進版控，要手改）
```

⚠ **git 只會搬被追蹤的檔**。合併後，舊資料夾 `linkedin-zip-challenge/` 裡還留著 `models/`、`datasets/`、`logs/`、`.env`、`.venv`，
要手動搬進 `thread-the-grid/`（`.venv` 不必搬，在新資料夾 `uv sync` 重建即可）。根目錄 `.gitignore` 刻意保留舊路徑的規則，
避免搬之前一個 `git add -A` 把 9.7 GB 的權重加進 git。2026-09-19 在 `zip-vlm` 實際走過一次：259 個追蹤檔全數到位、新 `.venv` 測試 344 passed。
...
| 專案／image | `threadgrid-app-<checkout>`／`threadgrid-app-<checkout>-threadgrid-app` | `threadgrid-dev-<checkout>`／`threadgrid-dev-<checkout>-threadgrid-app`（和正式版共用 5.858 GB 的層，並存不多佔空間）|
...
ollama 還在載模型，或容器沒起來。`docker logs -f threadgrid_ollama_server`；第一次呼叫會付模型載入時間（4B 模型約 1–2 分鐘）。
...
```
OLLAMA_MODEL_NAME=threadgrid-qwen35-4b-p4c:f16
...
或確認 volume 有掛上（`docker compose -p threadgrid-app-<checkout> exec threadgrid-app ls /app/models/rl_a2`）。
...
⚠ ollama 換成釘版本的標籤後，**下一次 `up` 會重建 `threadgrid_ollama_server` 容器**（volume 與模型都在，不會重下載模型）。
...
APP=threadgrid-app-<checkout>-threadgrid-app-1
