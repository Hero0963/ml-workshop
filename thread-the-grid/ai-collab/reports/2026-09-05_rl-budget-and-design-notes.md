...
## 6. `shaping_lambda=0` 對照，以及它意外量到的東西

handover §6 的第一件事。**結論：λ 沒有可量測的效果；但這個實驗真正的產出是 seed 雜訊地板。**

### 6.1 為什麼先做它

restart plan 的階段表指定一筆畫階段 shaping λ **＝ 0**（「只剩 +1 與 γ」），
而實作一直是 **0.2**（佔一局總分 14%），**從未關掉跑過**。
理論上 potential-based shaping 不改變最優策略（Ng et al., 1999 的 policy invariance），
但**不保證學習動態不變**——所以值得用 4 分鐘關掉試一次。

> ⚠ **誠實記一筆**：既然引了 policy invariance，就該同時預期**這是個 null result**。
> 它的價值是「用 4 分鐘關掉一個規格偏離的警報」，不是「找到提升」。
> 當初把它描述成「所有數字都量在沒人驗證過的設定上」的風險，**把期望值講高了**。

### 6.2 為此加的 CLI override

`train_maskable_ppo.py` 新增 `--shaping-lambda`，與 `--timesteps`／`--dataset`／`--n-envs` 同一個模式，
**不動 `train_config.py` 的預設值**（否則之後每個 run 都被改掉）。

⚠ `resolve_goal()` 必須用 `is not None` 而非真值判斷——**0.0 是假值**，
寫成 `if args.shaping_lambda:` 會讓這個實驗**靜默失效**（曲線全部正常、結果毫無差別）。
`test_shaping_lambda_zero_is_an_override_not_an_omission` 在守這件事。

### 6.3 結果：三個 seed × 兩臂

4×4、1M 步、資料集 `seed20300000_n20000_4-6`、held-out test（n=1,928，`deterministic=True`）。
**兩臂唯一的差別是 λ。**

| 臂 | seed 20260815 | seed 31415926 | seed 27182818 | 平均 | 臂內全距 |
|---|---|---|---|---|---|
| λ=0.2（現行） | **0.877** | 0.839 | 0.845 | **0.8537** | **0.0380** |
| λ=0（規格） | 0.839 | 0.833 | **0.863** | **0.8450** | **0.0300** |

```
between-arm difference of means : +0.0087
largest within-arm spread       :  0.0380
paired per-seed differences     : +0.038, +0.006, -0.018   ← 正負號會翻
greedy baseline across 6 runs   :  0.115-0.117 (spread 0.0020)
```

**兩臂平均只差 0.0087，而臂內自己就差 0.0380（4.4 倍），逐 seed 配對的差還會變號。**
⇒ **λ 在 4×4 上量不出差異。**

### 6.4 ★★ 真正的產出：seed 雜訊地板

**這是本 track 第一次量到「單 seed 結果的誤差量級」：4×4 約 ±0.02–0.04。**

而 greedy baseline 跨 6 個 run 只差 **0.002** ⇒ **評估端幾乎沒有雜訊**（與陷阱 #19「deterministic 重跑位元級相同」一致），
**波動幾乎全部來自訓練 seed。**

**這件事會回頭影響已經寫下的數字：**

- handover 表裡 4×4 的 **0.877 是三個 seed 裡最幸運的那個**，誠實的寫法是 **0.854 ± 0.038**。
  ⇒ **距離 0.90 的 done 條件比原本看起來的更遠。**
- 資料集加大那次的 **+0.089**（0.788 → 0.877）**大於雜訊，仍然站得住**。
- 6×6 的 **0.409 也是單 seed**。不能直接套用 4×4 的雜訊量級（盤面更難、curriculum 停在 k=30），
  但**同樣不能拿它跟 0.344 做 0.0x 等級的比較**——那次是 +0.065，接近雜訊量級，**應該視為「有動但未確證」**。

**⇒ 判讀規則（已寫進 handover §6）**：任何改動的效果**必須大於 seed 雜訊**才算數。
**0.0x 等級的改善不要報告成進步，也不要為了追那種量級去設計實驗。**

### 6.5 規格偏離的警報可以關掉

λ=0.2 這一側**沒有比較差**，所以既有那些數字（0.854、0.409）**不需要重做**，
`shaping_lambda=0.2` 保留即可。handover §0 ④ 與 §9 的那條缺口結案。
...
- **§5.6 的連通性特徵只量了成本，沒量效果。** 「+4.6%」是可以付的代價，
  但它會不會提升通關率**完全還沒測**。
- **§6.4 的雜訊地板只量在 4×4、每臂 3 個 seed。** 3 個點只夠給量級，不夠給信賴區間；
  **6×6 的雜訊完全沒量**（一次 5M 步約 20 分鐘，比 4×4 貴 5 倍）。
