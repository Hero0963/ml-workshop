...
**怎麼讀**：4×4 包了預算之後 PSO 一點都不差——中位數第 1 次就解出、最多 13 次、最慢 0.24 秒，
和 RL 的 best-of-32 同一個水準。**問題全在 6×6**：5 秒裡能試約 107 次，200 題只解出 6 題，
解出的那幾題中位數也要試 62 次。所以「不開放」的理由是 6×6，不是 4×4。

⚠ 兩邊的「次數」不對等：PSO 在 6×6 的 5 秒裡試了約 107 次，RL 最多 32 次；PSO 的第三欄是抽 200 題，RL 是整個 test split。
...
- 抽樣 200 題的 `served` 分數有抽樣誤差，§1 附的是 Wilson 95% 信賴區間。
- **兩欄彼此一致**：6×6 在 5 秒預算下總共跑了 21,407 次、解出 6 題，換算單次成功率約 0.00028，
  落在單次呼叫 0／2,000 的 95% 上界 0.0015 之內。
- `served` 於 2026-09-19 02:53 號誌空出來後量（先改成 `busy solvers`、跑完改回 `free`）：
  8 個 worker、127.0 秒，執行中取樣 CPU 36–44%（低於 75% 上限，含背景約 10%）。
