...
專案改名 **`thread-the-grid`**（原 `linkedin-zip-challenge`；LinkedIn 已申請 ZIP 商標）。本輪：Gradio 整併成兩個分頁、Svelte 補齊出題與讀圖、新增 `POST /api/puzzle/generate`、Gradio 升到 6.17.3、Apache-2.0 授權、主機端埠轉發修好並補驗。**總帳：[第二輪報告](reports/2026-09-19_wrap-up-round-2.md)**；決策與步驟：[計畫書](plans/2026-09-19_wrap-up-round-2.md)。
...
1. **把全部 solver 掛進 API** ← **✅ API／截圖端點／Gradio 已完成（2026-09-12，Track C，branch `feat/expose-heuristic-solvers`；2026-09-19 收尾：文件校正、rebase 到 `main`，並補上 Svelte 編輯器；**同日已合併進 `main` 並 push**）**
   ...
   - **2026-09-19 本人定案兩件事**：`budget_seconds` 請求欄位**不做**；**PSO 從 registry 拿掉**（程式、測試、量測都保留），上線的是九種。
     PSO 在 RL held-out 上的分數與理由見 [`reports/2026-09-19_pso-not-served.md`](reports/2026-09-19_pso-not-served.md)。
   - 十種都從 `src/core/solvers/registry.py` 上線。啟發式包一層 `_until_verified()`：重跑到答案通過
     ...
     **為什麼要包**：啟發式不管有沒有解出來都交回「看過最好的那條」，預設參數下 180 次呼叫只有 16 次（8.9%）是真解，不包就會把半成品畫成答案。
   ...
   - **✅ Svelte 下拉（2026-09-19，計畫書 C2）**：原本寫死 `DFS`／`A* (heapq)`／`CP-SAT`（連 RL 都沒有），
     改成開啟時打 `GET /api/solver/list`（由 `SOLVER_ENTRIES` 產生），依 kind 分組並顯示 note；放棄時顯示「No solution found」，
     不再放在「Solution」底下配兩張破圖。headless Chrome 端到端：**十種 10/10 畫出答案且通過 `is_solution`**
     （[artifacts](reports/artifacts/svelte-solver-list/)）；`test_registry.py` 守門：`Index.svelte` 出現 solver 名稱就失敗。
...
3. **RL solver 重啟** ← **★★ 2026-09-19：掃完 epoch、ExIt 第一輪訓練完成（單次嘗試 6×6 +0.125，單 seed）、判定器改嚴格；嚴格尺對照與 seed 沒做完，接手看 [handover-rl-solver.md](handover-rl-solver.md) §0**（兩個 goal 早在 2026-09-12 就達標）
   （原標題是「重啟前置研究」，2026-09-05 更名——前置研究早在 2026-08-15 就結束了）
   - **🎯 現況一句話**：**兩個 goal 都達標且有餘裕**——目前最好的模型 `bc_multi_456_e6`（BC 只訓練 6 epochs）
     在 best-of-32 是 4×4 **0.9953**、6×6 **0.9465**（門檻 0.90／0.85）。
     **★ 服務中的就是它**（2026-09-12 本人授權換掉 10 epochs 版；`solver_service.py`／`start.py`／[deployment-guide.md](deployment-guide.md) 三處一致）。
     舊的 `bc_multi_456`（6×6 0.8535 剛好踩線）留在原地沒刪，**只有呼叫端把 `attempts` 設成 1 時它才比較好**（6×6 best-of-1 0.4810 vs 0.4265），而 API 預設是 32。
     **A5 完成**：`RL (behaviour cloning)` 已是 API 的第 4 種 solver。
   ...
   - **★ 意外收穫：BC 本身在 6×6 嚴重過擬合**——deterministic 訓練集 **0.9005** vs 測試集 **0.5200**（4×4 只差 0.042），
     val 選擇準確率在 epoch 6 就到頂。⚠ 這不是重開已關掉的「繼續加資料」——那條線量的是 PPO 模型，不是 BC。
   - **★★ 順著上一條做的追加實驗，是這輪最大的一筆**：BC 只訓練 **6 epochs**（`bc_multi_456_e6`，同 seed 配對比較）
     ⇒ 6×6 **best-of-32 從 0.8535 變成 0.9465**（門檻 0.85）、每題嘗試次數 **7.76 → 5.24**；4×4 從 0.9917 變成 0.9953。
     但 **best-of-1 反而降**（0.4810 → 0.4265）⇒ **與 PPO 微調是同一條軸的兩端：訓練越久越尖、單次越準、多樣性越少。**
     **判定基準是 best-of-32 ⇒ 正確方向是「少訓練」。** ⚠ 單一 seed、**只量過 6 與 10 兩點**，最佳點可能更早。
   - **★★ 2026-09-12 收尾（本人定案：先不再訓練）**——掃 epoch 的**現成配方**在
     [handover-rl-solver.md](handover-rl-solver.md) §3：**訓練一次、每個 epoch 存一個 checkpoint 就拿齊所有點**
   ...
   - **★★ 2026-09-19（分支 `feat/rl-exit`，已合進 `main`；本人要求收尾時實驗做一半）**：
     ① **掃 epoch 完成**（寬鬆尺）：6×6 best-of-32 在 **e2–e5 是平台（0.9595–0.967）**，e6 開始下滑（0.9465）、e10 0.8535。
     ② **多解實測**：訓練題有別條解的比例 4×4 54%／5×5 75%／**6×6 87.5%**（出題器不檢查唯一解）。
     ③ **ExIt 第一輪訓練完成**：用服務模型在訓練集每題抽 32 次，收到 135,818 條別條解當額外標籤；
     **6×6 單次嘗試 0.6455 vs 同 epoch 的 BC 0.5205（+0.125）**，但**單 seed、寬鬆尺**。
     ④ **判定器全面改嚴格**（本人定案：路必須停在最大數字；7 處一起改，一支測試釘住）⇒ **量尺換了，舊數字都是寬鬆尺**（差約 −0.005）。
     **沒做完**：嚴格尺下的 BC vs ExIt 對照（只量完 e4 一對）與 seed 雜訊 ⇒ 指令與成本在 [handover-rl-solver.md](handover-rl-solver.md) §0.3。
     報告：[reports/2026-09-19_rl-epoch-sweep-and-exit.md](reports/2026-09-19_rl-epoch-sweep-and-exit.md)。
...
- Svelte 前端目前沒有自己的測試，值得補嗎？（2026-09-19：有一支 headless Chrome 端到端腳本
  `reports/artifacts/svelte-solver-list/drive_editor.py`，但沒進 pytest——它要先起服務與 Chrome。）
