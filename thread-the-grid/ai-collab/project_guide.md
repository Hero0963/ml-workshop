# 專案指南 (Project Guide) — thread-the-grid
...
│   │   ├── routers/             # echo.py、puzzle.py（出題）、solver.py、vision.py（端點）
...
│   ├── ui/
│   │   ├── gradio_app.py        # Gradio：Generate Puzzle ＋ Solve（上傳截圖／編輯器兩種模式）
...
- `POST /api/puzzle/generate` — 收 `{"size": 4|5|6}`，回 `GenerateResponse`（`grid_size`、`layout`、`walls`，與讀圖端點同形狀）；出題器限時放棄時自動重試最多 3 次（2026-09-19）。Gradio 與 Svelte 的出題都走它
- `POST /api/echo` — 連通性測試（Gradio 的 Echo 分頁 2026-09-19 拿掉，API 保留）
- `GET /docs` — Swagger
