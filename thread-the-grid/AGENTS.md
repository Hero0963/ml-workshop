# AGENTS.md — thread-the-grid 操作指南（子專案正本）
...
**Thread the Grid**：一筆畫路徑謎題——從 1 出發、依序經過每個號碼、走滿每一格、不穿牆（靈感來自網路上的 Zip 遊戲）。
用多種演算法解題、程序化出題、並包成可互動的網頁服務。**2026-09-19 前叫 `linkedin-zip-challenge`**（改名原因見 `ai-collab/plans/2026-09-19_wrap-up-round-2.md`）。
本質是**帶順序約束的 Hamiltonian path 問題**——精確解與啟發式解的取捨正是專案的主題。

含：9 種 solver、FastAPI 後端（含出題 API）、Gradio 主控台、Svelte Canvas 編輯器、出題器、GIF／PNG 視覺化、Docker 雙環境。
**VL 圖片解析（`src/core/vl_models/`）已在主流程上**（2026-08-29）：微調模型由本機 Ollama 服務，
接 `POST /api/vision/solve`、Gradio 的 Solve 分頁（Upload screenshot 模式）與 Svelte 的「Read screenshot」。
...
- **一律 `cd thread-the-grid` 之後才 `uv run`**；不要用 repo 根的 `.venv`（那是 py3.9 devtools）
