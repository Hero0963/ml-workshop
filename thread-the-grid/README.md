...
A grid path puzzle — one line through every cell, visiting the numbered waypoints in order —
taken end to end: **make** a board, **read** one out of a screenshot, and **solve** it. One
FastAPI application serves all three, with a Gradio console and a Svelte canvas editor on top,
and the whole stack starts with a single command. Inspired by the Zip puzzle games found online.
...
Captured on 2026-09-19 from the running stack.

| | |
|---|---|
| **Generate** a 4×4, 5×5 or 6×6 board, then open it in the editor | ![Generate Puzzle tab](./illustrations/ui/gradio-generate-puzzle.png) |
| **Read a screenshot**: the fine-tuned vision model reads the board (left), the solver draws the route (right) | ![Solve tab, upload mode](./illustrations/ui/gradio-solve-upload.png) |
| **Edit and solve**: numbers in the grid, walls as removable tags, here solved by the learned policy | ![Solve tab, editor mode](./illustrations/ui/gradio-solve-editor.png) |
| **Svelte canvas editor**: click a cell for a number, a border for a wall | ![Svelte editor](./illustrations/ui/svelte-editor-rl-solved.png) |
| **API** (`/docs`) | ![API docs](./illustrations/ui/api-docs.png) |
...
cd ml-workshop/thread-the-grid
python start.py
```
...
87.5% of the generated 6×6 training boards have more than one solution). `POST
/api/puzzle/generate` returns a 4×4, 5×5 or 6×6 board with walls, retrying the time-limited
search when it gives up; the Gradio console and the Svelte editor both use it. There is also a
...
| | |
|---|---|
| **Model chosen** | **Qwen3.5-4B** (Apache-2.0), fine-tuned with LoRA and served by Ollama as `threadgrid-qwen35-4b-p4c:f16` |
...
do not show that it reads screenshots of other versions of the game (six real ones: 5/6 end to
end — too few to claim). The synthetic held-out set is saturated at 1.000, so it can no longer tell two
...
a solution checks itself. Attempts stop at the first success, so the average cost is far
below 32.
...
| Windows: containers are healthy but `http://127.0.0.1:7440` does not answer | WSL's `networkingMode=mirrored` in `%USERPROFILE%\.wslconfig`, with an older Docker Desktop (seen with 4.32), leaves published ports with no listener on the host. Comment it out and run `wsl --shutdown` — verified on 2026-09-19. Newer Docker Desktop releases are reported to serve `localhost` in mirrored mode ([microsoft/WSL#41284](https://github.com/microsoft/WSL/issues/41284)); not tested here. |
...
│   │   ├── routers/          # echo / puzzle / solver / vision endpoints
...
The Gradio console at `/ui` has two tabs:

*   **Generate Puzzle** — a random 4×4, 5×5 or 6×6 board with walls. **Open in editor** takes
    it to the Solve tab.
*   **Solve**, in two modes:
    *   **Upload screenshot** — the vision model reads the board and a solver solves it; the
        answer carries warnings and a `solvable` flag. **Edit in editor** opens what was read,
        so a misread wall can be fixed before solving again.
    *   **Editor** — type numbers into the grid (`x` blocks a cell), add walls by their two
        cells, remove one with the × on its tag, and solve with any of the nine solvers.

The Svelte editor at `/svelte-ui/` does the same on a canvas: pick a size, then **Generate**
a board, start from a **Blank grid**, or read a screenshot; click cells and borders to change
it, and solve. `GET /api/echo/health` is
still there for health checks. Screenshots are in [`illustrations/ui/`](./illustrations/ui/).
...
cd thread-the-grid
uv sync
uv run pytest            # 344 passed, 8 xfailed on 2026-09-19, with the RL checkpoint in models/
uv run ruff check .
```

The eight `xfail`s are deliberate: they pin defects in the retired v1 environment, so an
unexpected pass means someone changed it. Without the RL checkpoint under `models/`, the one
...
Copyright 2025–2026 Hero0963. The code in this directory and the RL policy weights are
licensed under the [Apache License 2.0](./LICENSE). The vision model is a fine-tune of
Qwen3.5-4B, which is Apache-2.0 as well, and is published under the same license.

Not covered: the real game screenshots used for testing (`illustrations/puzzle_01.png` to
`puzzle_06.png`) belong to their respective owners, and the two sanity-check photos
(`illustrations/cat.png`, `illustrations/bird.png`) have no recorded source.
